...
**Checked 24 September 2026: is the "annual" `hc` series actually annual information?** No. `code/09_hc_interpolation_check.R` shows Rwanda's year-over-year `hc` change is nearly constant within each five-year block (1971–1975, 1976–1980, 1981–1985, 1986–1990, 1991–1993) and shifts distinctly at each five-year boundary — exactly Barro-Lee's observation grid. PWT's `hc` is a deterministic transform of Barro-Lee schooling data, interpolated between five-year points, not genuinely annual information. A 24-year pre-treatment window is therefore closer to 5 real data points per country than 24 independent ones, for every country in the donor pool alike — which is why the synthetic control's pre-treatment fit (RMSPE 0.00075, `code/07`) is so tight, and why 5 of the 27 donors fit even more tightly than Rwanda. This means a tight pre-treatment fit on `hc` does not validate the counterfactual the way Abadie's method assumes for a genuinely high-frequency outcome; the post-treatment gap may also be partly shaped by how the interpolation behaved for Rwanda specifically, not by a real annual trajectory. See `results/hc-interpolation-check.md`.
